...
def insert_variant(sequence, variant, allele_index, offset):
    #print(type(allele_index))
    start = variant.get_start() - offset
    end = variant.get_end() - offset
    included = sequence[0:start] + variant.get_allele(allele_index) + sequence[end:] #Added typecast to int
...
#    var_id = ';'.join(set([v.get_id() for v in variants if not v.is_absent()]))
    ...
    #print("REF ALLELE: ", ref_allele, start, end)
    # ploidy
    ...
    for allele, index in new_alleles.items():
        ...
        #print("ALT ALLELE: ", alt_alleles[index])
        if index != 0:
...
#### Here ,modification made for out files, sniffles is the second case here because it has a /
#### 0/0 GT is filtered out before, only 1 used as genotype --> prevent from error regarding phasing
#### I changed is_phased to true because it is needed to be True, It only takes the second number of the split 0/1, 1/1

    elif '/' in gt_string:
        is_phased = True #(changed to true)
        genot=gt_string.split(":")[0]
        allele=genot.split('/')[1]
        #print(allele)
        #for allele in gt_string.split('/'):
        if allele != '.':
            int_allele=int(allele)
            alleles.append(int_allele) #Removed (type cast to int) and added it in line before, only use first allele
        else:
            alleles.append(-1)
    else:
#        assert len(gt_string) == 1
        is_phased = True
        gt_string=(gt_string[0]) #MODIFIED HERE FOR MEDAKA (ADDED THIS LINE: only GT 1 present) 
        alleles = [int(gt_string)] if gt_string != '.' else [-1]
        
    return Genotype(alleles, is_phased)


def parse_line(samples, line, id_in_info=True):  
    variants = []
    fields = line.split()
    chrom = fields[0]
    start = int(fields[1])
    ...
    for i,s in enumerate(samples):
        ...
        #print(Variant([s], start, ref_allele, alt_alleles, [genotype], genotype.get_ploidy(), ids))
...
def print_cluster(variants, samples, chrom, reference):
    ...
    if combined is not None:
        ...
        if len(alt_alleles) < 1:
            ...
            return skipped_conflict, total_written
        if any(c not in 'CAGTcagt,' for c in ref_allele) or any(c not in 'CAGTcagt,"<INV>"' for c in alt_alleles):
            #print(alt_alleles)
    ...
    for filename in vcfs:
        ...
        for line in vcf:
            ...
            if line.startswith('#'):
                ...
                samples = fields[9:]
                continue
            chromosome, variants = parse_line(samples, line, False)
            """
            for v in variants:
                if chromosome=="JACEHA010000006.1":
                    if not v.is_absent():
                        print("Adding variants", v, "to chromosome:", chromosome)
        #variants_per_chromosome[chromosome].append(v)
                    else:
                        print("Variant", v, " is absent, not adding to chromosome:", chromosome)
            """            
...
#            variants_per_chromosome[chromosome] = combine_sorted_lists(variants_per_chromosome[chromosome], variants)
    ...
    for chrom in variants_per_chromosome:
        #print(chrom, variants_per_chromosome[chrom][0])
