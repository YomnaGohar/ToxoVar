...
def genotype_from_string(gt_string):
    ...
    if '|' in gt_string:
        ...
        allele=gt_string.split('/')[0]
        #for allele in gt_string.split('/'):
        if allele != '.':
            allele=int(allele)
            alleles.append(allele) #Removed (type cast to int) and added it in line before, only use first allele
        else:
            alleles.append(-1)
    else:
#        assert len(gt_string) == 1
        is_phased = True
        gt_string=(gt_string[0]) #MODIFIED HERE (ADDED THIS LINE: only GT 1 present) 
