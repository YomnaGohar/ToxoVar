# this is based on: getCompleteMycoFrequencies.pl
...
unless($samtools_bin)
{
	die "Please specify $samtools_bin";
}
...
{
	die "Please specify --referenceGenome"
}

#unless($regionName)
#{
#	die "Please specify --regionName"
#}
...
#die "Region '$regionName' not part of BAM/CRAM index" unless(exists $BAM_regions_lengths{$regionName});
...
my $samtools_command = qq($samtools_bin mpileup -B --reference $referenceGenome -d 100000 -q$minMappingQuality -Q$minBaseQuality $BAM);
print "Now executing:\n", $samtools_command, "\n";
...
while(<FREQ>)
{
	...
	print OUTPUT3 join("\t", $chr, $position, $refbase, join(';', map {$_ . '=' . $alleles_counts{$_}} keys %alleles_counts)), "\n";
}
close(FREQ);	
