ruleorder: figure_overlap > getting_statistics_from_vep > overlapping_vep > overlapping_var
ruleorder: figure_in_coding_regions_ovelap > getting_statistics_from_vep > overlapping_vep > overlapping_var

rule generate_figures_overlapping:
  input:
   expand("../analysis/Graph/graph_construction/variation_overlap/{var}.{length}/0000.vcf", var=["SNP","INS","DEL"],length=["less_than_50","greater_than_50"]),
   expand("../analysis/Graph/graph_construction/variation_overlap/{var}.{length}/variant_consequences.all.pdf", var=["SNP","INS","DEL"],length=["less_than_50","greater_than_50"]),
   expand("../analysis/Graph/graph_construction/variation_overlap/{var}.{length}/variant_consequences_in_coding_areas.pdf", var=["SNP","INS","DEL"],length=["less_than_50","greater_than_50"])
...
rule figure_overlap:
  ...
  output:
    o= "../analysis/Graph/graph_construction/variation_overlap/{var}.{length}/variant_consequences.all.pdf"
  shell:
    """
...
rule figure_in_coding_regions_ovelap:
  ...
  output:
    o= "../analysis/Graph/graph_construction/variation_overlap/{var}.{length}/variant_consequences_in_coding_areas.pdf"
  shell:
    """
    python3 scripts/variant_barplot.py {output.o} {input.f} 
    """            